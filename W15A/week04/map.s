...
points_loop_cond:
	...
	# &my_points[i] = my_points + i * sizeof(element)
	mul	$t6, $t0, SIZEOF_POINT	# $t6 = i * sizeof(element)
	add	$t6, $t6, my_points
	lw	$t1, ROW_OFFSET($t6)	# t1 = int row = my_points[i].row;
	lw	$t2, COL_OFFSET($t6)	# t2 = int col = my_points[i].col;

	# &topography_grid[row][col] = grid + (MAP_SIZE * row + col) * SIZEOF(int)
	mul	$t6, $t1, MAP_SIZE	# t6 = row * MAP_SIZE
	add	$t6, $t6, $t2		# t6 = .... + col
	mul	$t6, $t6, 4		# $t6 = offset in bytes

	lw	$t3, topography_grid($t6) # int height = topography_grid[row][col];
