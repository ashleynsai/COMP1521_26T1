...
2. The following snippet of C code declares two variables, `s1` and `s2`, with some subtle differences between them.
   ...
   Variable that can be accessed from anywhere within the program. Stored in data

   - How do they differ from local variables? Where are they each located in memory?

   They are stored in the stack, and they can only be accessed from the function they're declared in. 

   - What is a string literal? Where are they located in memory?

   Hardcoded string, typically you can't change the values after declaration
